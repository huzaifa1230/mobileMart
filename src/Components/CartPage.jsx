          ...
          <>
            {cart.map((item) => (
              <div key={item.id} className="flex items-center justify-between mb-4 border-b border-gray-700 pb-4">
                <img className="w-20 h-20 rounded" src={item.img} alt={item.title} />
            ...
            <div className="flex justify-between items-center mt-4">
              <span className="text-2xl font-bold text-white">Total: ${calculateTotal()}</span>
              <button className="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2 text-center">
                Proceed to Checkout
              </button>
            </div>
          </>
